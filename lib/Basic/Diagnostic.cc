...
static const StaticDiagInfoRec *GetDiagInfo(unsigned DiagID) {
  ...
  if (Found == StaticDiagInfo + NumDiagEntries || Found->DiagID != DiagID)
    return 0;
...
Diagnostic::~Diagnostic() { delete CustomDiagInfo; }
...
/// object, classify the specified diagnostic ID into a Level, consumable by
/// the DiagnosticClient.
Diagnostic::Level Diagnostic::getDiagnosticLevel(unsigned DiagID,
                                                 unsigned DiagClass) const {
  ...
  case diag::MAP_WARNING:
    ...
    if (ExtBehavior == Ext_Error && (MappingInfo & 8) == 0 &&
...
bool Diagnostic::setDiagnosticGroupMapping(const char *Group,
  ...
  if (Found == OptionTable + OptionTableSize || strcmp(Found->Name, Group) != 0)
...
static void HandleSelectModifier(unsigned ValNo, const char *Argument,
...
static void HandlePluralModifier(unsigned ValNo, const char *Argument,
                                 unsigned ArgumentLen,
                                 llvm::SmallVectorImpl<char> &OutStr) {
  const char *ArgumentEnd = Argument + ArgumentLen;
  while (1) {
...
/// array.
void DiagnosticInfo::FormatDiagnostic(
