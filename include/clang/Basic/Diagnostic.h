...
public:
  ...
  enum ExtensionHandling { Ext_Ignore, Ext_Warn, Ext_Error };
...
public:
  ...
  void setExtensionHandlingBehavior(ExtensionHandling H) { ExtBehavior = H; }
  ...
  void ConvertArgToString(ArgumentKind Kind, intptr_t Val, const char *Modifier,
                          unsigned ModLen, const char *Argument,
                          unsigned ArgLen,
  ...
  /// This only works on builtin diagnostics, not custom ones, and is not legal
  /// to call on NOTEs.
...
private:
  ...
  void setDiagnosticMappingInternal(unsigned DiagId, unsigned Map,
                                    bool isUser) const {
    if (isUser)
      Map |= 8; // Set the high bit for user mappings.
...
inline DiagnosticBuilder Diagnostic::Report(FullSourceLoc Loc,
                                            unsigned DiagID) {
...
public:
  ...
  const CodeModificationHint *getCodeModificationHints() const {
    return DiagObj->NumCodeModificationHints
               ? &DiagObj->CodeModificationHints[0]
               : 0;
  }
